...
class Agent:
    ...
    MAX_ACTION_NORM = 0.015
    NUMBER_OF_STEPS_BEFORE_TARGET_UPDATE = 50
    ...
    def __init__(self):
        ...
        self.set_angles_actions = np.arange(0, 360, 30)  # angles of possible actions
    ...
    def get_next_action(self, state):
        ...
        discrete_action = np.random.choice(np.arange(len(self.set_angles_actions)), 1, p=np.array([(1 - self.epsilon) if i == q_values_tensor.max(0)[1].item() else (self.epsilon / (len(self.set_angles_actions) - 1)) for i in range(len(self.set_angles_actions))]))[0]
    ...
    def set_next_state_and_distance(self, next_state, distance_to_goal):
        ...
        if self.num_steps_taken % Agent.NUMBER_OF_STEPS_BEFORE_TARGET_UPDATE == 0:
    ...
    def _discrete_action_to_continuous(self, discrete_action):
        angle = math.radians(self.set_angles_actions[discrete_action])
...
class DQN:
    ...
    def _calculate_loss(self, transitions):
        ...
        inputs = np.array([np.append(transitions[i, 0], transitions[i, 1]) for i in range(transitions.shape[0])])
        rewards = np.array([np.array([(transitions[i, 2])]) for i in range(transitions.shape[0])])
        next_state_inputs = np.array([np.append(transitions[i, 3], self._discrete_action_to_continuous(j)) for j in range(0, len(self.set_angles_actions)) for i in range(transitions.shape[0])])
        ...
        next_state_q_values = self.target_q_network.forward(next_state_inputs_tensor)
        #print(next_state_inputs.reshape(50, 12))
        print('next state', next_state_q_values.max(0)[0].item())

        loss = torch.nn.MSELoss()(network_q_values, batch_rewards_tensor + self.discount * next_state_q_values.max(0)[0].item())
        return loss

    # Function to convert discrete action (as used by a DQN) to a continuous action (as used by the environment).
    def _discrete_action_to_continuous(self, discrete_action):
        angle = math.radians(self.set_angles_actions[discrete_action])
        continous_action = np.array(
